...
### 6a. The keyword baseline was previously circular (now corrected)
In the initial dev evaluation, the golden seed was generated by the keyword labeller — evaluating the keyword heuristic on its own outputs gave 100% accuracy, which was meaningless. After the full human review of all 200 rows and adding the iCloud+backup disambiguation rule, this circularity is substantially broken: the keyword heuristic now scores **0.785 accuracy / 0.735 macro-F1** on genuinely human-assigned labels. However, ~70% of labels that the human reviewer confirmed unchanged are still keyword-aligned, so the keyword baseline score is still somewhat inflated relative to a fully independent evaluation.
