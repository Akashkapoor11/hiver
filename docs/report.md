...
|--------|:--------:|:--------:|:-----------:|
| Majority baseline (`sync_setup_data`) | 0.100 | 0.018 | 0.018 |
| Keyword heuristic | 1.000 ⚠️ | 1.000 ⚠️ | 1.000 ⚠️ |
| **Proposed agent (TF-IDF + SGD)** | **0.605** | **0.587** | **0.587** |

> ⚠️ **Keyword accuracy = 1.0 is a circular artefact.** The dev golden seed was built using the keyword labeller, so the keyword heuristic is evaluated against its own outputs. See §7 for full disclosure.
...
|--------|------:|
| Accuracy | 0.840 |
| Macro-F1 (both classes) | 0.535 |
| Escalate precision | **0.982** |
| Escalate recall | 0.851 |

High precision on escalate means the agent almost never sends an autonomous reply to a case that should have had human attention — which is the correct asymmetry for a safety-first support system. The lower recall (14.9% of true escalate cases auto-handled) represents the main residual risk.
...
At macro-F1 = 0.587, the agent looks moderately good. But `features_accessibility_other` (the catch-all) likely has near-zero precision, and `battery_power` likely has very high precision. Averaging across 10 intents with wildly different difficulty masks the worst failures.

### 6d. 84% escalation accuracy sounds good but hides the dangerous error
The 15% of true-escalate cases that were auto-handled (false negatives on escalation) each represent a potential autonomous reply to a case that should have had human review. In a live system handling thousands of tweets per day, 15% missed escalations is a significant safety gap.
